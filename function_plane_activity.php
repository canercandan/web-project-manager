...
define('SQL_GET_DATE_PROJECT_START', 'SELECT day(project_date), month(project_date), year(project_date) 
FROM tw_project WHERE project_id = \'%d\';');

define('SQL_DIFF_DATE_PROJECT_END', 'SELECT day(project_date_end), month(project_date_end), year(project_date_end) 
FROM tw_project WHERE project_id = \'%d\';');
