...
define('POST_PROJECT_HOUR_DAY', 'projhourday');
...
define('FIELD_PROJECT_HOUR_DAY', '<field_hour_day value="%d">%s</field_hour_day>');

define('ELM_INFORMATION_PROJ',
       '<editable>1</editable>
	<name post="%s">%s</name>
...
define('SQL_NEW_INFORMATION',
       'SELECT	f.project_name, f.project_describ,
		day(f.project_date), month(f.project_date), year(f.project_date),
		day(f.project_date_end), month(f.project_date_end), year(f.project_date_end),
		p.profil_name, p.profil_fname, title_name, project_hour_per_day
	FROM	tw_project f
...
define('SQL_NEW_UPDATE_PROJECT',
	...
	SET	project_name = \'%s\',
		...
		project_hour_per_day = \'%d\'
	WHERE	project_id = \'%d\';');

define('SQL_NEW_ADD_PROJECT',
       'INSERT INTO tw_project
	(project_id, project_autor_usr_id, project_name, project_describ,
	 project_date, project_date_end, project_hour_per_day)
	VALUES (NULL, \'%d\', \'%s\', \'%s\', DATE(\'%04d-%02d-%02d\'),
		DATE(\'%04d-%02d-%02d\'), \'%d\');');
